            ...
            <Grid container spacing={2} justifyContent="center" sx={{ paddingY: 0 }} mt={5}>
              ...
              <Grid item xs={10} sx={{ pl: 5}}>
                ...
                {value?.fileName&&<Typography>{value?.fileName}</Typography>}
                  ...
                  <Box max-width="80.33%" sx={{ p: 5, display: 'flex', justifyContent: 'center'}}>
                    <TextField label="Describe the crime in as much detail as possible..." multiline rows={8} variant="outlined" sx={{ borderRadius: 'none'}} value={value?.description||""} onChange={(e)=>setValue({...value,description:e.target.value})}/>
                  </Box>
                </Box>
              </Grid>
            </Grid>
